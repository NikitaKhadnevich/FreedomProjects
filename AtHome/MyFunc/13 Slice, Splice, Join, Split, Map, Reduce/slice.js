...
//Метод  Reduce - более сложный гибрид map который используется для вычесления единого значения на основе всего массива. Reduce - "Бог" перебирающих методов.
let arrReduce = [5, 34, 32, 2, 5, 10, 9]
let result = arrReduce.reduce( function(accum, item, index) {
   return accum + item
}, 0); //l(result)
// В первый раз значение accum равно 0, затем каждый раз при итерации массива происходит перезапись значения accum
// 1)accum = 0 + 5 = 5
// 2)accum = 5 + 34 = 39
// 3)accum = 39 + 32 = 71...

// let result2 = arrReduce.reduce((accum, item, index) => {
//    return accum + item
// }, 0); l(result2)

// let sum = 0
// for (let index = 0; index < arrReduce.length; index++) {
//    sum = sum + arrReduce[index]
// }; l(sum)

let sum2 = 0
for (let index = 0; index < arrReduce.length; index++) {
   if (sum2 < arrReduce[index])
   sum2 = arrReduce[index]
}; //l(sum2)

let result3 = arrReduce.reduce( function(accum, item, index) {
   if (accum < item) {
      return item
   }
   else {
...
let result4 = arrReduce.reduce((accum, item, index) => {
   if (accum < item) {
      return item
   }
   else {
...
let result5 = arrReduce.reduce((accum,item,index) => {
   if (item > accum[1]) { // выбираем 1 эл-т массива 
      return [index, item]
   }
   else {
      return accum
   }
}, [0, arrReduce[0]]) // задаем в качестве аккумулятора 0 айтим и 0 индекс
l(result5)
